...
def parse_args():
    parser = argparse.ArgumentParser(description="SCIM dataset sentence classification")
    parser.add_argument(
        "--input_dataset",
        type=str,
        default="data/processed/scim",
        help='Path to the training dataset (default "data/processed/scim")',
    )
    parser.add_argument(
        "--model",
        type=str,
...
def main(args):
    ...
    train_dataset = scim_to_sentences(dataset["train"])
    ...
    validation_dataset = scim_to_sentences(dataset["validation"])
    ...
    trainer.train()
    pred = (
        test_tokenized_dataset.remove_columns(["sentences", "labels", "context"])
        .with_format("torch")
